...
public:
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        assert(size_ && pos.node_ && pos.node_->next_node);
        const Node* tmp = pos.node_->next_node;
    ...
    SingleLinkedList(std::initializer_list<Type> values) {
        CopySingleLinkedList(values);
    }

    SingleLinkedList(const SingleLinkedList& other) {
        CopySingleLinkedList(other);
    }
...
private:
    ...
    void CopySingleLinkedList(Container& other) {
        ...
        for (const auto& value : other) {
            const_iterator = tmp.InsertAfter(const_iterator, value);
        }
        swap(tmp);
    }
};
...
bool operator==(const SingleLinkedList<Type>& lhs, const SingleLinkedList<Type>& rhs) {
    if (lhs.cbefore_begin() == rhs.cbefore_begin()){
        return true;
    }
